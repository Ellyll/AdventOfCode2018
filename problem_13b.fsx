...
let moveCarts carts grid =
    let rec loop remaining currentCarts =
        ...
        | [] -> currentCarts
        | cart::rest ->
            let otherCarts = currentCarts |> List.filter (fun c -> c <> cart)
            let newRemaining, newCarts =
                match moveCart cart otherCarts grid with
                | CrashedIntoOtherCart target ->
                    // Remove crashed carts
                    rest |> List.filter (fun c -> c <> target),
                    otherCarts |> List.filter (fun c -> c <> target)
                | CartMoveResult.Success movedCart ->
                    rest, movedCart::otherCarts
            loop newRemaining newCarts
    let sortedCarts = carts |> List.sortBy (fun c -> (c.Y,c.X))
    loop sortedCarts carts

let getResult grid carts =
    let rec loop currentCarts =
        match moveCarts currentCarts grid with
        | [] -> failwith "No carts left"
        | [lastCart] -> (lastCart.X, lastCart.Y)
        | newCarts -> loop newCarts
    loop carts

let input = System.IO.File.ReadAllLines "problem_13.data"
let grid, carts = input |> linesToGrid

let result = getResult grid carts
